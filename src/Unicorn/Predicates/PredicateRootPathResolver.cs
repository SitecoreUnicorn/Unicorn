...
namespace Unicorn.Predicates
{
	public class PredicateRootPathResolver
	{
		...
		public IItemData[] GetRootSerializedItems()
		{
			...
			{
				var item = TargetDataStore.GetByPath(include.Path, include.DatabaseName).ToArray();
				...
				{
					_logger.Error("Unable to resolve serialized item for included root path {0}:{1}. The item does not exist in {2}. It has been skipped. Perhaps you need to perform an initial serialization from the control panel?".FormatWith(include.DatabaseName, include.Path, TargetDataStore.FriendlyName));
				}
				else
				{
					_logger.Error("Multiple serialized items matched included root path {0}:{1} in {2}. You cannot have a root path that is not unique. It has been skipped.".FormatWith(include.DatabaseName, include.Path, TargetDataStore.FriendlyName));
				}
			}
