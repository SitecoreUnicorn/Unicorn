...
namespace Unicorn.ControlPanel.Controls
{
	internal class ConfigurationInfo : IControlPanelControl
	{
		...
		public void Render(HtmlTextWriter writer)
		{
			...
			var configurationHasDependents = ControlPanelUtility.HasDependents(Configuration);
			...
			if (configurationHasDependents)
			{
				writer.Write(@"<div class=""help"">");
				writer.Write(@"<p>The following configurations are depending on this configuration:</p>");
				writer.Write(@"<ul>");
				foreach (var dependent in ControlPanelUtility.FindConfigurationsDependents(Configuration))
				{
					writer.Write($@"<li>{dependent.Name}</li>");
				}
				writer.Write(@"</ul>");
				writer.Write(@"<p>Please sync this configuration before the dependents.</p>");
				writer.Write(@"</div>");
			}
